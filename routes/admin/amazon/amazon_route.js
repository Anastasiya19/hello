...
var amazon_Brands_Model = require('../../../models/amazon_brands_schema');
...
router.get('/shortlist_asins/pagenumber/:pagenumber', function (req, res) {
  console.log("Page requested is: ", req.params.pagenumber);
    ...
    exec(function (err, mobiles_array) {
        ...
        res.render(('shortlist_asins'),{mobiles_array: mobiles_array});
    });
});
...
router.post('/disable_asin',function (req, res) {
  ...
  var query = {"product_id.retailer_product_id": req.body.asin};
  var options = {multi: false};
  amazon_Mobiles_Model.update(query, {"internal_codes.status": 0}, options, function (err, result) {
        if (err) throw err;
      res.json({
        result: "Disabled",
        asin: req.body.asin
...
router.post('/enable_asin', function (req, res) {
  ...
  var query = {"product_id.retailer_product_id": req.body.asin};
  var options = {multi: false};
  amazon_Mobiles_Model.update(query, {"internal_codes.status": 1}, options, function (err, result) {
        if (err) throw err;
...
//This will only fetch the latest brands added to the amazon_brands_model
router.get('/shortlist_brands', function (req, res) {
    ...
    exec(function (err, brands_array) {
      console.log("Render request received on the Server");
      if (err) throw err;
      res.render(('shortlist_amazon_brands'),{brands_array: brands_array});
    });
...
router.post('/disable_brand',function (req, res) {
  ...
  var options = {multi: false};
  amazon_Brands_Model.update(query, {"internal_codes.status": "0"}, options, function (err, result) {
        if (err) throw err;
      res.json({
        result: "Disabled",
...
router.post('/enable_brand',function (req, res) {
  ...
  var query = {"brand_name": req.body.brand_name};
  var options = {multi: false};
  amazon_Brands_Model.update(query, {"internal_codes.status": "1"}, options, function (err, result) {
        if (err) throw err;
      res.json({
        result: "Enabled",
        brand: req.body.brand_name
...
//Route to match amazon products with potential products in compared db
router.get('/match_products/:brand_name', function (req, res) {

  //Fetch each mobile to be matched - Store it in mobile_to_be_matched variable
  var query = amazon_Mobiles_Model.findOne({"internal_codes.status": "11111", "product_basic_info.productBrand_normalized": req.params.brand_name});
...
router.post('/match_true', function (req, res) {
  ...
  //Select the product to be matched UUID. This is the product from Amazon database
  ...
  //Find the product_tobematched in the amazon mobiles db and fetch its data
  //which will be updated in the combined mobiles db
  amazon_Mobiles_Model.findOne({"product_id.uuid": product_tobematched_uuid}, function(err, product_tobematched){
    ...
    mobiles_Compared_Model.update({"product_id.uuid":shortlisted_uuid}, {$push: {"product_retailers": update}},{safe: true},function(err,results){
      ...
      else
      {
        ...
        //Update the amazon_mobiles_model with the uuid of the shortlisted product
        amazon_Mobiles_Model.update({"product_id.uuid": product_tobematched_uuid}, {$set: {"internal_codes.status": "111111", "compared_db.product_uuid": shortlisted_uuid}}, function(err, results){
        ...
        });//amazon_Mobiles_Model.update ends
  ...
  });//amazon_mobiles_model findOne ends
...
router.post('/match_false', function (req, res) {
  ...
  mobiles_Compared_Model.update({"product_id.uuid":shortlisted_uuid}, {$pull: {"product_retailers": update}},{safe: true},function(err,results){
    ...
    else
    {
      ...
      //Update the amazon mobiles model internal_codes and compared_db reference
      amazon_Mobiles_Model.update({"product_id.uuid": product_tobematched_uuid}, {$set: {"internal_codes.status": "11111", "compared_db.product_uuid": "N/A"}}, function(err, results){
...
router.post('/match_no_match', function (req, res) {
  ...
  amazon_Mobiles_Model.findOne({"product_id.uuid": product_tobematched_uuid}, function(err, product){
    console.log("Inside Mobiles_model.find");
   // console.log("This is product_tobecompared", product);
    ...
    var product_retailers_document = {
      ...
      //setting the final_positions to be the product_position of the anchor retailer
      mobiles.final_position = product.product_position;
    ...
    mobiles.save(function(err)
    {
      if (err){ 
        ...
        var options = {multi: false};
        amazon_Mobiles_Model.update(query, {"compared_db.product_uuid": mobiles.product_id.uuid, "internal_codes.status": "211111"}, options, function (err, resp) {
          if (err) {
            console.log(err);
          }
...
router.post('/skip_match', function (req, res) {
  ...
  amazon_Mobiles_Model.update({"product_id.uuid": product_tobeskipped_uuid}, {$set: {"internal_codes.status":"011111" , "compared_db.product_uuid": "N/A"}}, function(err, results){
