...
//1.2 Load the models
var paytm_Mobiles_Model = require('../../../models/paytm_mobiles_schema');
var paytm_Brands_Model = require('../../../models/paytm_brands_schema');
...
router.post('/disable_asin', function(req, res) {
  ...
  paytm_Mobiles_Model.update(query, {
...
router.get('/shortlist_brands', function (req, res) {
    ...
    exec(function (err, brands_array) {
        console.log("Render request received on the Server");
        if (err) throw err;
        res.render(('shortlist_paytm_brands'),{brands_array: brands_array});
    });
...
router.post('/disable_brand',function (req, res) {
  ...
  paytm_Brands_Model.update(query, {"internal_codes.status": "0"}, options, function (err, result) {
        if (err) throw err;
      res.json({
        result: "Disabled",
...
router.post('/enable_brand',function (req, res) {
  ...
  var query = {"brand_name": req.body.brand_name};
  var options = {multi: false};
  paytm_Brands_Model.update(query, {"internal_codes.status": "1"}, options, function (err, result) {
        if (err) throw err;
      res.json({
        result: "Enabled",
        brand: req.body.brand_name
...
//Route to match paytm products with potential products in compared db
router.get('/match_products', function (req, res) {

  //Fetch each mobile to be matched - Store it in mobile_to_be_matched variable
  var query = paytm_Mobiles_Model.findOne({"internal_codes.status": "1111", "product_basic_info.productBrand_normalized": "Apple"});
...
router.post('/match_true', function (req, res) {
  ...
  //Select the product to be matched UUID. This is the product from paytm database
  ...
  //Find the product_tobematched in the paytm mobiles db and fetch its data
  //which will be updated in the combined mobiles db
  paytm_Mobiles_Model.findOne({"product_id.uuid": product_tobematched_uuid}, function(err, product_tobematched){
    ...
    mobiles_Compared_Model.update({"product_id.uuid":shortlisted_uuid}, {$push: {"product_retailers": update}},{safe: true},function(err,results){
      ...
      else
      {
        ...
        //Update the paytm_mobiles_model with the uuid of the shortlisted product
        paytm_Mobiles_Model.update({"product_id.uuid": product_tobematched_uuid}, {$set: {"internal_codes.status": "11111", "compared_db.product_uuid": shortlisted_uuid}}, function(err, results){
        ...
        });//paytm_Mobiles_Model.update ends
  ...
  });//paytm_mobiles_model findOne ends
...
router.post('/match_false', function (req, res) {
  ...
  mobiles_Compared_Model.update({"product_id.uuid":shortlisted_uuid}, {$pull: {"product_retailers": update}},{safe: true},function(err,results){
    ...
    else
    {
      ...
      //Update the paytm mobiles model internal_codes and compared_db reference
      paytm_Mobiles_Model.update({"product_id.uuid": product_tobematched_uuid}, {$set: {"internal_codes.status": "1111", "compared_db.product_uuid": "N/A"}}, function(err, results){
...
router.post('/match_no_match', function (req, res) {
 ...
 var product_tobeskipped_uuid = req.body.product_tobeskipped_uuid;
  ...
  paytm_Mobiles_Model.update({"product_id.uuid": product_tobeskipped_uuid}, {$set: {"internal_codes.status":"01111" , "compared_db.product_uuid": "N/A"}}, function(err, results){
