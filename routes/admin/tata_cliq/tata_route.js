...
//1.2 Load the models
var tata_cliq_Mobiles_Model = require('../../../models/tata_cliq_mobiles_schema');
var tata_cliq_Brands_Model = require('../../../models/tata_cliq_brands_schema');
...
//Load the tata_cliq apac module configuration
...
router.get('/shortlist_asins/pagenumber/:pagenumber', function(req, res) {
  ...
  exec(function(err, mobiles_array) {
    ...
    res.render(('shortlist_tata_cliq'), {
...
router.post('/disable_asin', function(req, res) {
  ...
  tata_cliq_Mobiles_Model.update(query, {
...
router.get('/shortlist_brands', function(req, res) {
  ...
  exec(function(err, brands_array) {
    console.log("Render request received on the Server");
    if (err) throw err;
    res.render(('shortlist_tata_cliq_brands'), {
...
router.post('/disable_brand', function(req, res) {
  ...
  tata_cliq_Brands_Model.update(query, {
    "internal_codes.status": "0"
  }, options, function(err, result) {
    if (err) throw err;
    res.json({
...
//Route to match tata_cliq products with potential products in compared db
router.get('/match_products', function(req, res) {

  //Fetch each mobile to be matched - Store it in mobile_to_be_matched variable
  var query = tata_cliq_Mobiles_Model.findOne({
    "internal_codes.status": "1111",
    "product_basic_info.productBrand_normalized": "Samsung"
  });
  query.exec(function(err, mobile_to_be_matched) {
    ...
    if (err) {
      ...
      if (!mobile_to_be_matched) {
        res.send("no mobiles")
      } else {
        //Fetch all the mobiles from the compared model belonging to the same brand as mobile_to_be_matched
        var query = mobiles_Compared_Model.find({
          "product_basic_info.productBrand_normalized": mobile_to_be_matched.product_basic_info.productBrand_normalized
        });
        query.exec(function(err, compared_model_results) {
          if (err) {
            ...
            res.render(('product_match'), {
              results_array: results_array,
              product_tobematched: mobile_to_be_matched
            });
...
router.post('/match_true', function(req, res) {
  ...
  //Select the product to be matched UUID. This is the product from tata_cliq database
  ...
  //Find the product_tobematched in the tata_cliq mobiles db and fetch its data
  //which will be updated in the combined mobiles db
  tata_cliq_Mobiles_Model.findOne({
    "product_id.uuid": product_tobematched_uuid
  }, function(err, product_tobematched) {
    ...
    mobiles_Compared_Model.update({
      "product_id.uuid": shortlisted_uuid
    }, {
      $push: {
        "product_retailers": update
      }
    }, {
      safe: true
      ...
      if (err) {
        ...
        //Update the tata_cliq_mobiles_model with the uuid of the shortlisted product
        tata_cliq_Mobiles_Model.update({
          "product_id.uuid": product_tobematched_uuid
        }, {
          $set: {
            "internal_codes.status": "11111",
            "compared_db.product_uuid": shortlisted_uuid
          }
        }, function(err, results) {
        ...
        }); //tata_cliq_Mobiles_Model.update ends
  ...
  }); //tata_cliq_mobiles_model findOne ends
...
router.post('/match_false', function(req, res) {
  ...
  mobiles_Compared_Model.update({
    ...
    if (err) {
      ...
      //Update the tata_cliq mobiles model internal_codes and compared_db reference
      tata_cliq_Mobiles_Model.update({
        "product_id.uuid": product_tobematched_uuid
      }, {
        $set: {
          "internal_codes.status": "1111",
          "compared_db.product_uuid": "N/A"
        }
      }, function(err, results) {
...
router.post('/match_no_match', function(req, res) {
  ...
  tata_cliq_Mobiles_Model.update({
    "product_id.uuid": product_tobeskipped_uuid
  }, {
    $set: {
      "internal_codes.status": "01111",
      "compared_db.product_uuid": "N/A"
    }
  }, function(err, results) {
